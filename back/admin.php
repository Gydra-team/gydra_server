...
if(isset($_POST["search"])){
  ...
  $query = "SELECT * FROM hydradmins WHERE tema LIKE `%$search%` ORDER by id DESC";
...
<?php
if(!empty($_GET[`id`])){  
  $query="SELECT * FROM hydradmins WHERE id=$_GET[id]";
  $result = mysql_query($query);
  $row = mysql_fetch_array($result);
}
// Добавление
if(isset($_POST[`insert`])){
  $query="INSERT INTO hydradmins (tema,text) VALUES (`$_POST[tema]`,`$_POST[text]`)";
  mysql_query($query);
}
//Корректировка
if(isset($_POST[`edit`])){
  $query="UPDATE hydradmins SET tema=`$_POST[tema]`,text=`$_POST[text]`  WHERE id=`$_POST[id]`";
  $result = mysql_query($query);
  unset($row);  
}
//Удаление
